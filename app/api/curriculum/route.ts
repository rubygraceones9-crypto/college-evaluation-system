...
export async function buildCurriculum() {
  // Ensure table exists safely (PostgreSQL style)
  await query(`
      ...
      for (const yr of Object.keys((curriculum as any)[prog])) {
        for (const sem of Object.keys((curriculum as any)[prog][yr])) {
          for (const subj of (curriculum as any)[prog][yr][sem]) {
            await query(`
              INSERT INTO subjects (code, name, program, year_level, semester) 
              VALUES (?, ?, ?, ?, ?) 
              ON CONFLICT (code, program) DO NOTHING`, 
  ...
  const rows = await query('SELECT * FROM subjects WHERE is_archived = FALSE ORDER BY program, year_level, semester, code');
